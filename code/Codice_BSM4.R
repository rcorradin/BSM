...
# -----------------------------
# The data reported in the chapter for the LMM
# and the corresponding design matrices
# -----------------------------
...
gibbs_LMM <- function(y, X, U, c, niter, nburn, 
  ...
  for(i in 1:niter){
    ...
    for(l in 1:n){
      ...
      out_pred_dens[i,l] <- dnorm(y[l], mean = temp_pred, 
                                  sd = sqrt(sigma2))
    }
  }
...
# marginal M2
out_coefs_M2 <- logistic_M2[[1]]
log_post_M2 <- function(pars, data){
  y <- data$d[,1]
  X <- data$d[,-1]
  p <- length(pars)
  out <- dmvnorm(pars, rep(0, p), diag(10^3, p), log = T)
  for(l in 1:nrow(X)){
    temp_probs <- exp(X[l,] %*% pars) / (1 + exp(X[l,] %*% pars))
    out <- out + ifelse(y[l] == 1, log(temp_probs), log(1 - temp_probs))
  }
  return(out)
}
lb <- rep(-Inf, 3)
ub <- rep(Inf, 3)
names(lb) <- names(ub) <- colnames(out_coefs_M2) <- c("par1", "par2", "par3")

mar_M2 <- bridge_sampler(samples = out_coefs_M2,
               log_posterior = log_post_M2,
               data = list(n = nrow(X), d = cbind(y, X[,1:3])),
               lb = lb, 
               ub = ub)
# marginal M3
out_coefs_M3 <- logistic_M3[[1]]
log_post_M3 <- function(pars, data){
  y <- data$d[,1]
  X <- data$d[,-1]
  p <- length(pars)
  out <- dmvnorm(pars, rep(0, p), diag(10^3, p), log = T)
...
lb <- rep(-Inf, 2)
ub <- rep(Inf, 2)
names(lb) <- names(ub) <- colnames(out_coefs_M3) <- c("par1", "par2")

mar_M3 <- bridge_sampler(samples = out_coefs_M3,
                         log_posterior = log_post_M3,
                         data = list(n = nrow(X), d = cbind(y, X[,1:2])),
                         lb = lb, 
                         ub = ub)

# test

BF23 <- exp(mar_M2$logml - mar_M3$logml)
BF23
...
bridge_sampler(out_STAN1)
bridge_sampler(out_STAN2)
bridge_sampler(out_STAN3)

exp(bridge_sampler(out_STAN2)$logml - bridge_sampler(out_STAN3)$logml)
